  ...
  (table-test-case T "manual tests ported from legilogic_lib"
    ...
    (test-case "zipping"
      ...
      (let-match ([focus . context] (F .find-path k (F .singleton k v)))
        ...
        (check-equal? (F .element?/path context) #t)
        (check-equal? (F .unzip (cons focus context)) (F .singleton k v))))
    (test-case "zipping 2"
      (def k 101)
      (def v "needle")
      ...
      (let-match ([focus . context] (F .find-path k t))
        (check-equal? focus (Leaf v))
        (check-equal? (F .element?/path context) #t)
        (check-equal? (F .unzip (cons focus context)) t))
      (let-match ([focus . context] (F .find-path 0 (E .empty)))
        (check-equal? focus (Empty))
        (check-equal? (F .element?/path context) #t)
        (check-equal? (F .unzip (cons focus context)) (E .empty)))
